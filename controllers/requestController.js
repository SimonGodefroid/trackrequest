            ...
            .exec()
              ...
              .then(user =>{
                ...
                .then(request => {
                  ...
                  if(request.votes.upvotes[req.params.id]){
                    // if already upvoted we undo
                    _.unset(request, `votes.upvotes[${req.params.id}]`);
                  }else if(request.votes.downvotes[req.params.id]){
                    // if already downvoted we undo the downvote
                    _.unset(request, `votes.downvotes[${req.params.id}]`)
                    request.votes.upvotes[req.params.id] = true;
                  }else if(!request.votes.upvotes[req.params.id]){
          ...
          downvote: (req, res) => {
            ...
            .exec()
              ...
              .then(user =>{
                ...
                .exec()
                .then(request => {
                  if(request.votes.downvotes[req.params.id]){
                    ...
                    _.unset(request, `votes.upvotes[${req.params.id}]`)
                    // and we downvote
                    request.votes.downvotes[req.params.id] = true;
                  }else if(!request.votes.downvotes[req.params.id]){
                    // if it wasn't downvoted nor upvoted we downvote
                    request.votes.downvotes[req.params.id] = true;
                  }
                  console.log('uppies',Object.keys(request.votes.upvotes).length)
                  console.log('dpwnsies',Object.keys(request.votes.downvotes).length)
                  const query = { _id: req.params.id };
                  const update = { 
                    votes: request.votes, 
                    upvotes: Object.keys(request.votes.upvotes).length,
